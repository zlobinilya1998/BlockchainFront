            ...
            <div v-else-if="symbols" class="symbols-wrapper">
                <div class="symbol" v-for="(symbol,index) in symbols" :key="index">
                    <div style="flex: 1 1 auto">
                        ...
                        <p style="max-width: 250px">Минимальное кол-во для заказа: {{symbol.min_order_size.toLocaleString()}}</p>
                        <div>Статус: <span :style="{color: getStatus(symbol.status).color}" v-html="getStatus(symbol.status).text"/></div>
                    </div>
                    <c-btn v-if="symbol.status === Status.open" inverted block style="padding: 10px;cursor: pointer;margin: 20px 0 0">
                        Приобрести
                        <c-icon v-html="'ci-tag'"/>
                    </c-btn>
                </div>
            </div>
        </div>
